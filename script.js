...
function isWindowSmall() {
  ...
  function scrollTo(event) { // слежу за прокруткой колеса
      if (event.deltaY > 5 || event.keyCode === 40) {         //если колесо крутится вниз или нажата кнопка вниз
          if (blockNumber !== blockScrolling.length - 1) blockNumber++;
      }
      else if (event.deltaY < -5 || event.keyCode === 38) { // если колесо крутится вверх или нажата кнопка вверх
